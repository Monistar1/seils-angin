...
(function() {
  ...
  // Merge runtime package config if the ConfigEngine has loaded.
  function mergeRuntimeConfig() {
    const cfg = window.AppConfig?.config;
    if (!cfg) return;
    CONFIG.transitionDuration = cfg.ui?.transition_duration_ms ?? CONFIG.transitionDuration;
    CONFIG.prefetchNextPage = Boolean(cfg.media?.preload_next_page);
  }
  ...
  function whenConfigReady(cb) {
    if (window.AppConfig?.ready) {
      ...
      window.AppConfig.on('ready', cb);
    } else {
      // ConfigEngine not present — run with defaults
      cb();
    }
  }
  ...
  function detectCapabilities() {
    const userReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    const packageNoAnim = window.AppConfig?.config?.ui?.animations === 'none';
    state.reducedMotion = userReduced || packageNoAnim;
  ...
  function initPageTransitions() {
    ...
    document.querySelectorAll('a[href]').forEach(link => {
      ...
      // Skip links disabled by the package config engine
      if (link.classList.contains('nav-disabled')) return;
  ...
  function initPrefetch() {
    if (CONFIG.prefetchNextPage === false) return;

    // Prefetch next likely pages based on current page, filtered by package config
    ...
    (nextPages[page] || [])
      .filter(href => {
        ...
        return !window.AppConfig || window.AppConfig.isPageEnabled(target);
      })
      .forEach(href => prefetchPage(href));
  }
  ...
  /* ==========================================================
     ...
     ========================================================== */
  function initPetals() {
    // Respect package feature flags
    if (window.AppConfig && !window.AppConfig.isFeatureEnabled('petals')) return;
